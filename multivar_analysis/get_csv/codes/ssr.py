...
for region in list(regions.keys()):
    ...
    for month in list(range(len(month_days))):
        ...
        # 5th-degree polynomial fit for SSR
        coefficients_ssr = np.polyfit(df_extent_resampled_monthly.index, bworth_filt_ssr, 5)
        bfl_values_ssr = np.polyval(coefficients_ssr, df_extent_resampled_monthly.index)

        curr_corr, p_value = pearsonr(bworth_filt_seaice, bworth_filt_ssr)
        ...
        ax.plot(df_nc_resampled_monthly.index, bworth_filt_ssr, label='Surface Radiation', color='r')
        ax.plot(df_nc_resampled_monthly.index, bfl_values_ssr, label='Radiation Best Fit', color='y')
